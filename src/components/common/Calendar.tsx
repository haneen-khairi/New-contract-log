...
export default function Calendar({ contracts }: Props) {
  ...
  useEffect(() => {
    if (calendarApi && contracts.length > 0) {
      calendarApi.gotoDate(`${contracts[0].date}`);
  ...
  return (
    <Box ref={boxRef} id="sadijnasijdnasjidnnijas">
      <Flex gap={"12px"}>
        <IconButton
          onClick={goPrev}
          icon={<IoChevronBack />}
        ...
        >
          {Array.from({ length: 12 }, (_, i) => i + 1).map((month) => (
            <option key={formatMonth(month)} value={formatMonth(month)}>
              {formatMonth(month)}
            </option>
          ))}
        </Select>
      ...
      <FullCalendar
        ...
        initialDate={contracts.length > 0 ? contracts[0].date : new Date()}
        dayMaxEventRows={3}
        nowIndicator={true}
        contentHeight={650}
        eventDisplay={"block"}
        selectable={true}
        // eventSources={contracts}
